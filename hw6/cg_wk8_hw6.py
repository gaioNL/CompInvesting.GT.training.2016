...
def find_events(ls_symbols, d_data):
    ...
    for s_sym in ls_symbols:
        for i in range(1, len(ldt_timestamps)):
            ...
            # specifically, if bollinger[t-1]>=-2 & bollinger[t]<-2 && bollinger[SPY][t]>=1.1
            if bolVal_yest >= -2 and bolVal_today < -2 and bolVal_SPY_today>=1.1 :
...
if __name__ == '__main__':
    ...
    ep.eventprofiler(df_events, d_data, i_lookback=20, i_lookforward=20,
                s_filename='WK8_Bol_2012.pdf', b_market_neutral=True, b_errorbars=True,
                s_market_sym='SPY')